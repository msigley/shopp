 ...
 * AdminTable.php
 *
 * Provides a base admin table class for rendering a list of items in an AJAXified HTML table
 *
 * @copyright WordPress, August 2010
 ...
 */
abstract class ShoppAdminTable extends ShoppRequestFramework {
	...
	public function search_box( $text, $input_id ) {
		...
		echo '<p class="search-box">'
		   ...
		   . '	<input type="search" id="' . $input_id . '" name="s" value="' . _admin_search_query() . '" />'
...
<?php
	...
	public function display() {
		...
		echo  '<table class="wp-list-table ' . implode( ' ', $this->get_table_classes() ) . '"' . ( empty($this->id) ? '' : ' id="' . esc_attr($this->id) . '"' ) . '>'
			. '	<thead>'
			. '	<tr>';
	...
	/**
	 * Generate an editor UI for the current item
	 *
	 * @since 3.1.0
	 * @access public
	 * @abstract
	 ...
	 **/
	public function editorui( $markup = null ) {
		if ( ! empty($markup) )
			$this->editor = $markup;
		return $this->editor;
	}
